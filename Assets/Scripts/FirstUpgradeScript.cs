...
public class FirstUpgradeScript : MonoBehaviour
{
    ...
    public void Upgrade()
    {
        switch (Upgrades[player.UpgradeNum])
        {
            case 0:
    ...
    void Update()
    {
        ...
        if (UpgradeScreen.activeInHierarchy)
        {
            ...
            if (Upgrades[player.UpgradeNum] == 0) { UpgradeText.text = "AttackPower\n+1"; UpgradeIcons[0].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 1) { UpgradeText.text = "AttackPower\n+3"; UpgradeIcons[1].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 2) { UpgradeText.text = "AttackPower\n+5"; UpgradeIcons[2].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 3) { UpgradeText.text = "HP\n+5"; UpgradeIcons[6].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 4) { UpgradeText.text = "HP\n+15"; UpgradeIcons[7].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 5) { UpgradeText.text = "HP\n+30"; UpgradeIcons[8].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 6) { UpgradeText.text = "Bullet Speed\n+0.1"; UpgradeIcons[3].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 7) { UpgradeText.text = "Bullet Speed\n+0.3"; UpgradeIcons[4].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 8) { UpgradeText.text = "Bullet Speed\n+0.5"; UpgradeIcons[5].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 9) { UpgradeText.text = "HP Regen\n+0.5f"; UpgradeIcons[9].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 10) { UpgradeText.text = "HP Regen\n+1f"; UpgradeIcons[10].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 11) { UpgradeText.text = "HP Regen\n+3f"; UpgradeIcons[11].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 12) { UpgradeText.text = "Penetration\n+1/4"; UpgradeIcons[12].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 13) { UpgradeText.text = "Multishot\n+1/4"; UpgradeIcons[13].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 14) { UpgradeText.text = "Defence\n+1"; UpgradeIcons[14].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 15) { UpgradeText.text = "Defence\n+2"; UpgradeIcons[15].SetActive(true); }
            if (Upgrades[player.UpgradeNum] == 16) { UpgradeText.text = "Defence\n+2"; UpgradeIcons[16].SetActive(true); }
            reset = true;
        }
    }
}
